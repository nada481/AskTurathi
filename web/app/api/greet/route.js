...
export async function POST(request) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: { systemInstruction: getGreetingPrompt(language), maxOutputTokens: 120 },
    });

    const greeting = response.candidates?.[0]?.content?.parts?.[0]?.text?.trim();
    if (!greeting) throw new Error('No greeting generated');

    const { buffer, mime } = await synthesizeSpeech(greeting, language);
    console.log(`[greet] total: ${Date.now() - t0}ms`);
